...
function CustomerReportModal({ isOpen, onClose }) {
  ...
  const searchCustomer = async () => {
    ...
    setLoading(true);
    try {
      const { data } = await api.get(`/reports/customer-search?customer_name=${encodeURIComponent(customerName)}`);
      setReportData(data);
    } catch (err) {
        ...
        <div className="p-6 border-b border-gray-100">
          <div className="flex space-x-3">
            ...
            <button
              ...
              disabled={loading}
              className="flex items-center space-x-2 bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-xl font-semibold text-sm transition-all disabled:opacity-50"
            >
              {loading ? (
        ...
        {reportData && (
          <div className="p-6">
            ...
            <div className="bg-gradient-to-br from-indigo-50 to-purple-50 rounded-xl p-6 mb-6 border border-indigo-100">
              <h3 className="text-lg font-bold text-gray-900 mb-4">Customer: {reportData.customer_name}</h3>
              <div className="grid grid-cols-3 gap-4">
                <div className="bg-white rounded-lg p-4 text-center">
                  <p className="text-xs font-bold text-gray-500 uppercase">Transactions</p>
                  <p className="text-2xl font-black text-gray-900 mt-1">{reportData.total_transactions}</p>
                </div>
                <div className="bg-white rounded-lg p-4 text-center">
                  <p className="text-xs font-bold text-gray-500 uppercase">Total Quantity</p>
                  <p className="text-2xl font-black text-gray-900 mt-1">{reportData.total_qty}</p>
                </div>
                <div className="bg-white rounded-lg p-4 text-center">
                  <p className="text-xs font-bold text-gray-500 uppercase">Total Amount</p>
                  <p className="text-2xl font-black text-indigo-600 mt-1">Rs {reportData.total_amount.toLocaleString()}</p>
                </div>
              </div>
            ...
            {/* Product Summary */}
            {reportData.product_summary.length > 0 && (
              <div className="mb-6">
