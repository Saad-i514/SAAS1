...
export default function Reports() {
  ...
  const downloadPDF = async (type) => {
    try {
      ...
      if (type === 'products') {
        ...
        let tq=0,ts=0,tp=0;
        rows = data.map(t => { tq+=t.quantity||0; const a=t.debit||0; if(t.type==='sale')ts+=a; else if(t.type==='purchase')tp+=a; return [t.date?new Date(t.date).toLocaleDateString():'—',(t.type||'—').toUpperCase(),t.product_name||'—',t.quantity||0,(t.unit_price||0).toFixed(2),(t.discount||0).toFixed(2),a.toFixed(2),t.order_no||'—']; });
  ...
  const handlePrint = () => {
    ...
    const rows = toPrint.map(i => `<tr><td>${i.date?new Date(i.date).toLocaleDateString():'—'}</td><td>${i.product_name||'—'}</td><td>${i.category||'—'}</td><td style="text-align:center">${i.quantity||0}</td><td style="text-align:right">Rs ${(i.unit_sale_price||0).toFixed(2)}</td><td style="text-align:right">Rs ${(i.total_sale_price||0).toFixed(2)}</td><td style="text-align:right">Rs ${(i.total_cost_price||0).toFixed(2)}</td><td style="text-align:right;color:${i.profit>=0?'#059669':'#dc2626'}">Rs ${(i.profit||0).toFixed(2)}</td></tr>`).join('');
                    ...
                    {isExpanded && group.items.map((item, rowIdx) => {
                      ...
                      return (
                        <tr key={item._idx} className={`border-t border-gray-50 dark:border-slate-800/60 transition-colors ${isSel ? 'bg-indigo-50/20 dark:bg-indigo-900/5' : rowIdx % 2 === 0 ? '' : 'bg-gray-50/30 dark:bg-slate-800/20'}`}>
                          ...
                          <td className="text-gray-500 dark:text-slate-400 tabular">{item.date ? new Date(item.date).toLocaleDateString() : '—'}</td>
